...
import yahoofinance.{Stock => YFStock}
import yahoofinance.quotes.stock.{StockQuote => YFStockQuote}
...
class FakeStockQuoteGenerator(symbol: StockSymbol) extends StockQuoteGenerator {
  ...
  def newQuote(lastQuote: StockQuote): StockQuote = {
    val stock: YFStock = YahooFinance.get(lastQuote.symbol.toString)
    val quote: YFStockQuote = stock.getQuote(true)
    logger.info(s"got quote for $stock $quote")
    StockQuote(symbol, StockPrice(quote.getPrice().doubleValue()))
  }
}
